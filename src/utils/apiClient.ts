...
export async function apiClient<T = unknown>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      throw new ApiClientError(getFriendlyErrorMessage(response.status), {
      ...
      const isNetworkError = err.name === 'TypeError' || !(err as ApiClientError).status;

      // Handle retry for network errors/timeouts
      if ((isNetworkError || isAbortError) && attempt < maxRetries) {
        const delay = baseDelay * Math.pow(2, attempt);
        console.warn(
          `API request failed due to ${isAbortError ? 'timeout' : 'network error'}. Retrying in ${delay}ms... (Attempt ${attempt + 1}/${maxRetries})`
        );
        await sleep(delay);
        continue;
      }
      ...
      const friendlyMessage = getFriendlyErrorMessage(
        isAbortError ? 408 : (err as ApiClientError).status,
        isNetworkError
      );

      lastError = new ApiClientError(friendlyMessage, {
        status: isAbortError ? 408 : (err as ApiClientError).status,
        isNetworkError,
        retryCount: attempt,
      });
...
export const sorobanRpc = {
  post: <T = unknown>(method: string, params: unknown = []): Promise<T> => {
    return apiClient<T>(SOROBAN_RPC_URL, {
      method: 'POST',
      headers: {
